...
export const MarcaForm: React.FC = () => {
  ...
  useEffect(() => {
    if (isEdit && id) {
      fetchRequest(marcaService.getById(parseInt(id)), {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const apiCall = isEdit 
      ? marcaService.update(parseInt(id!), formData)
      : marcaService.create(formData);

    await request(apiCall, {
      successMessage: `Marca ${isEdit ? 'actualizada' : 'creada'} correctamente`,
      onSuccess: () => navigate('/inventario/marcas'),
    });
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          <form onSubmit={handleSubmit}>
            ...
            <FormField label="Nombre de la Marca *" id="marca">
              <input
                type="text"
                name="marca"
                id="marca"
            ...
            <div className="mt-6 flex justify-end space-x-3">
              <button
                type="button"
                onClick={() => navigate('/inventario/marcas')}
