...
export const CategoriasPage: React.FC = () => {
  ...
  const fetchCategorias = useCallback(async () => {
    ...
    request(categoriaService.getAll(params), {
  ...
  const handleDelete = async () => {
    ...
    await deleteRequest(categoriaService.delete(selectedCategoria.id), {
  ...
  const handleAdd = () => {
    navigate('/inventario/categorias/nuevo');
  };

  const handleEdit = (categoria: Categoria) => {
    navigate(`/inventario/categorias/${categoria.id}/editar`);
  };

  const columns: Column<Categoria>[] = [
    { header: 'ID', accessor: 'id', sortable: true },
    { header: 'Categoría', accessor: 'categoria', sortable: true },
    { header: 'Estado', accessor: (c) => <Badge status={c.estado} /> },
    {
      header: 'Acciones',
      accessor: (c) => (
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden p-6">
      ...
      <div className="flex-shrink-0 bg-white p-4 rounded-lg shadow mb-4 flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0">
        <SearchInput onSearch={setSearchTerm} placeholder="Buscar categoría..." />
      ...
      >
        <p className="text-sm text-gray-500">
          ¿Estás seguro de que deseas eliminar la categoría <span className="font-bold">{selectedCategoria?.categoria}</span>? 
