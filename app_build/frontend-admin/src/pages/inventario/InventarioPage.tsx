...
export const InventarioPage: React.FC = () => {
  ...
  const columns: Column<Medicamento>[] = [
    ...
    { header: 'Categoría', accessor: (med) => med.categoria_detalle?.categoria || '-' },
    { 
      header: 'Presentación', 
      accessor: (med) => {
        const p = med.presentacion_detalle;
        if (!p) return '-';
        return `${p.forma_farmaceutica_detalle?.forma} · ${p.concentracion} · ${p.unidad_medida_detalle?.abreviatura}`;
      }
    },
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden p-6">
      ...
      <div className="flex-shrink-0 bg-white p-4 rounded-lg shadow mb-4 flex flex-col sm:flex-row sm:items-center justify-between space-y-4 sm:space-y-0">
        <SearchInput onSearch={setSearchTerm} placeholder="Buscar por nombre, marca, presentación, stock o precio..." />
